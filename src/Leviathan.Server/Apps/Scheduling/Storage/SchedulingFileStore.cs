...
public sealed class SchedulingFileStore(IConfiguration config) : SchedulingStore
{
    ...
    public Task SaveNotification(ScheduledNotification notification, CancellationToken ct = default) => Write(Path.Combine(ProviderDir(notification.ProviderId), "bookings", notification.BookingId.Value, "notifications", notification.Id.Value + ".json"), notification, ct);
    public async Task<ScheduledNotification?> GetNotification(NotificationId id, CancellationToken ct = default) { if (!Directory.Exists(_root)) return null; foreach (var file in Directory.EnumerateFiles(_root, id.Value + ".json", SearchOption.AllDirectories)) return await Read<ScheduledNotification>(file, ct); return null; }
    public Task<IReadOnlyList<ScheduledNotification>> GetNotifications(ProviderId providerId, BookingId bookingId, CancellationToken ct = default) => ReadAll<ScheduledNotification>(Path.Combine(ProviderDir(providerId), "bookings", bookingId.Value, "notifications"), "*.json", ct);
